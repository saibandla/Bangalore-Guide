...
{
    destinationField.text=destaddr;
}
...
-(void)getJsonData
{
    ...
    if(![address.text isEqualToString:@""] &&![destinationField.text isEqualToString:@""])
    {
        
        NSString *str=[[NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/directions/json?origin=%@&destination=%@&key=AIzaSyDrlg-8NIkc-Eziz1Wkb3uV_890Y5XLpeo&mode=transit&departure_time=now&alternatives=true",address.text,destinationField.text] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] ;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        destinationField.text=[tableView cellForRowAtIndexPath:indexPath].textLabel.text;
        [address resignFirstResponder];
    }
...
- (void)fetchedData2:(NSData *)responseData {
    ...
    if(responseData!=nil)
    {
        ...
        if([destinationField.text isEqualToString:@""]&&textFlag==4)
        {
            Locationsarray=@[];
